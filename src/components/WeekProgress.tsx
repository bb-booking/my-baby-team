    ...
    <div className="card-soft flex flex-col items-center text-center gap-4 section-fade-in">
      <div className="relative w-36 h-36">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-xs text-muted-foreground">{t("weekProgress.ofWeeks", { total: totalWeeks })}</span>
        </div>
      </div>

      <div className="flex items-center gap-3 bg-sand-light rounded-2xl px-5 py-3">
        <img src={babySizeImg} alt={t("weekProgress.babySize")} className="w-10 h-10 object-contain" />
        <div className="text-left">
          <p className="text-sm text-muted-foreground">{t("weekProgress.babyIsNowLike")}</p>
      ...
      <span className="inline-block text-xs font-medium bg-sage-light text-foreground px-3 py-1 rounded-full">
        {t("weekProgress.trimesterBadge", { num: trimester })}
      </span>
    </div>
  );
}
