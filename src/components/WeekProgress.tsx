...
export function WeekProgress() {
  const progress = (currentWeek.week / currentWeek.totalWeeks) * 100;
    ...
    <div className="card-soft flex flex-col items-center text-center gap-4 section-fade-in">
      ...
      <div className="relative w-36 h-36">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-3xl font-serif">{currentWeek.week}</span>
          <span className="text-xs text-muted-foreground">af {currentWeek.totalWeeks} uger</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 bg-sand-light rounded-2xl px-5 py-3">
        ...
        <div className="text-left">
          ...
          <p className="font-medium">{currentWeek.babySize} {currentWeek.babySizeEmoji}</p>
        </div>
      </div>

      {/* Trimester badge */}
      <span className="inline-block text-xs font-medium bg-sage-light text-foreground px-3 py-1 rounded-full">
        {currentWeek.trimester}. trimester
      </span>
    </div>
  );
}
