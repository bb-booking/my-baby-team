...
export default function BabynavnePage() {
  ...
  const genderOptions = [
    { key: "dreng" as const, label: "Dreng", emoji: "👦" },
    { key: "pige" as const, label: "Pige", emoji: "👧" },
    { key: "neutral" as const, label: "Neutral", emoji: "🌟" },
  ];
  ...
  return (
    <div className="space-y-5">
      <div className="section-fade-in">
        ...
        <p className="label-upper mt-1">JERES FAVORITNAVNE</p>
      </div>

      <div className="card-soft section-fade-in" style={{ animationDelay: "80ms" }}>
        <p className="label-upper mb-3">TILFØJ NAVN</p>
        <input
          type="text"
          value={newName}
        ...
        <button onClick={addName} disabled={!newName.trim()} className="btn-moss w-full disabled:opacity-50">
          <Plus className="w-4 h-4 inline mr-1" /> Tilføj navn
        </button>
      </div>

      {names.length > 0 && (
        <div className="card-soft section-fade-in" style={{ animationDelay: "160ms" }}>
          <p className="label-upper mb-3">GEMTE NAVNE ({names.length})</p>
          {names.map(n => (
            <div key={n.id} className="flex items-center gap-3 py-2.5 border-b border-foreground/5 last:border-0">
              ...
              <div className="flex-1">
                ...
                <p className="text-[0.6rem] text-muted-foreground capitalize">{n.gender}</p>
              </div>
